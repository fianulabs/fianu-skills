---
name: summarizing-evidence
description: Use when producing a JSON summary of a Fianu finding, violation, or attestation for the in-product evidence panel. Strict schema, no markdown fences, no editorializing.
---
...
Load when producing a one-shot JSON summary of a Fianu finding, violation,
or attestation for the in-product evidence panel. This skill is atomic —
it does NOT load other skills. The output is rendered next to raw
evidence in the Fianu console; the reader can see the data, your job is
to make the *meaning* fast to grasp and surface concrete actions worth
taking.

You are summarizing for a security-aware reader who already understands
Fianu primitives (attestations, findings, violations, controls, gates).
